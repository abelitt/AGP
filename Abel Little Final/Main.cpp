...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYDOWN:
		...
		if (wParam == VK_NUMPAD3)
			g_pCam->Pitch(-5.0f);
		if (wParam == VK_DELETE)
			g_pModel->IncPos(0, 0, -1);
		if (wParam == VK_END)
			g_pModel->IncPos(0, 0, 1);
		if (wParam == VK_UP)
			g_pModel->IncPos(0, -1, 0);
		if (wParam == VK_DOWN)
			g_pModel->IncPos(0, 1, 0);
		if (wParam == VK_LEFT)
			g_pModel->IncPos(-1, 0, 0);
		if (wParam == VK_RIGHT)
			g_pModel->IncPos(1, 0, 0);
		if (wParam == VK_HOME)
		{
			g_pModel->LookAt_XZ(g_pCam->GetX(), g_pCam->GetZ());
			//g_pModel->LookAt_XZ(90, 10);
		}
		if (wParam == VK_INSERT)
		{
			g_pModel->MoveForward(0.5f);
		}
...
HRESULT InitialiseGraphics()
{
	...
	*/
	...
	g_pModel->LoadModel((char*)"assets/Cube.obj");
	g_pModel->AddTexture((char*)"assets/heart_cube.bmp");
	//g_pModel->SetRotationZ(90);
...
void RenderFrame(void) //UPDATE
{
	...
	g_pModel->Draw(viewptr, projectionptr);
	delete viewptr;
	viewptr = nullptr;
	...
	ss << "X- " << g_pModel->GetPosX() << " Y-" << g_pModel->GetPosY() << " Z-" << g_pModel->GetPosZ();
